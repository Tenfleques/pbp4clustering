...
def main():
    parser = argparse.ArgumentParser(description="Run PBP on WDBC (3x10) and report metrics.")
    ...
    X_mats, y, meta = load_wdbc_matrices()
    print(f"Loaded WDBC: matrices={X_mats.shape}, labels={y.shape}, m×n={meta['matrix_shape']}")
    ...
    if args.plot:
        out_png = str(Path(args.results_dir) / f"wdbc_targets_pbp_{args.agg}.png")
        scatter_features(X_pbp, y, out_png, title=f"WDBC PBP (agg={args.agg}) - Diagnosis", label_names=meta.get("labels_map"))
