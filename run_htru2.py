...
def main():
    parser = argparse.ArgumentParser(description="Run PBP on HTRU2 (2x4) and report metrics.")
    parser.add_argument("--data-dir", default="./data/htru2", help="Directory containing HTRU_2.csv")
    ...
    X_mats, y, meta = load_htru2_matrices(args.data_dir)
    print(f"Loaded HTRU2: matrices={X_mats.shape}, labels={y.shape}, m×n={meta['matrix_shape']}")
    ...
    if args.plot:
        out_png = str(Path(args.results_dir) / f"htru2_targets_pbp_{args.agg}.png")
        scatter_features(X_pbp, y, out_png, title=f"HTRU2 PBP (agg={args.agg}) - Class", label_names=meta.get("labels_map"))
