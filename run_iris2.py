...
def main():
    parser = argparse.ArgumentParser(description="Run PBP on Iris with 2x2 matrices and report metrics.")
    ...
    X_mats, y, meta = load_iris_matrices()
    print(f"Loaded Iris: matrices={X_mats.shape}, labels={y.shape}, m×n={meta['matrix_shape']}")
    ...
    if args.plot:
        out_png = str(Path(args.results_dir) / f"iris_targets_pbp_{args.agg}.png")
        scatter_features(X_pbp, y, out_png, title=f"Iris PBP (agg={args.agg}) - Species", label_names=meta.get("labels_map"))
