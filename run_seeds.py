...
def main():
    parser = argparse.ArgumentParser(description="Run PBP on UCI Seeds (3x2) and report metrics/plot.")
    parser.add_argument("--data-dir", default="./data/seeds", help="Directory containing seeds.csv")
    ...
    X_mats, y, meta = load_seeds_matrices(args.data_dir)
    print(f"Loaded Seeds: matrices={X_mats.shape}, labels={y.shape}, m×n={meta['matrix_shape']}")
    ...
    if args.plot:
        out_png = str(Path(args.results_dir) / f"seeds_targets_pbp_{args.agg}.png")
        scatter_features(X_pbp, y, out_png, title=f"Seeds PBP (agg={args.agg}) - Class", label_names=meta.get("labels_map"))
