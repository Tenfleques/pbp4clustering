...
def main():
    parser = argparse.ArgumentParser(description="Run PBP on UCI Wine Quality with 2/3-row inputs and report metrics.")
    parser.add_argument("--data-dir", default="./data/wine", help="Download/cache directory for Wine")
    parser.add_argument("--agg", default="sum", help="Aggregation function name for PBP")
    parser.add_argument("--row-blocks", type=int, choices=[2, 3], default=3, help="Aggregate feature rows into this many blocks before PBP")
    ...
    print(f"Loaded Wine: matrices={X_mats.shape}, labels={y.shape}, m×n={meta['matrix_shape']}")

    # Optional: aggregate features into blocks
    ...
    out_png = str(Path(args.results_dir) / f"wine_targets_pbp_{args.agg}.png")
    scatter_features(X_pbp, y, out_png, title=f"Wine PBP (agg={args.agg}) - True Targets", label_names=meta.get("labels_map"))
